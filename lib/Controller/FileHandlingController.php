		...
		try {
			if($path !== '' && (is_int($mtime) && $mtime > 0)) {
				...
				if($mtime !== $filemtime) {
					...
					$this->logger->error('File: ' . $path . ' modified since opening.', ['app' => 'mindmap_app']);
					return new DataResponse(
					...
					if($file->isUpdateable()) {
						...
						// Not writeable!
						$this->logger->error('User does not have permission to write to file: ' . $path,['app' => 'mindmap_app']);
				...
				$this->logger->error('No file mtime supplied', ['app' => 'mindmap_app']);
