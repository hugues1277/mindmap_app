<?php
namespace OCA\MindMap_App\Migration;
...
class InstallStep implements IRepairStep {
    ...
    */
    public function run(IOutput $output) {
      ...
      // create mime icon      
      try {    
        //$filetypesDir = dirname(\OCP\Template::mimetype_icon('text'));
        $filetypesDir = './core/img/filetypes';
        if(file_exists ($filetypesDir) && !file_exists ($filetypesDir.'/application-jm.svg')){
          echo file_put_contents($filetypesDir.'/application-jm.svg', '<?xml version="1.0"?>
          <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" width="512px" height="512px" viewBox="0 0 31.416 31.416" style="enable-background:new 0 0 31.416 31.416;" xml:space="preserve"><g><g>
            <g>
      ...
      $this->appendToFile($mimetypealiasesFile, ['application/jm' => 'application-jm']);
      $this->appendToFile($mimetypemappingFile, ['jm' => ['application/jm']]);
      $this->logger->info("Add .jm to mimetype list.", ["app" => "mindmap_app"]);
